...
public class TC_63861_VerifyProjectApprovalProcessByChoosingApprovalTypeAsDualEitherWhenApprover1_ApprovesAndApprover2_RejectsTheProject extends TestBase {

    @Test(dataProviderClass = TestDataProvider.class, dataProvider = "getData", description = "ProjectManagement")
    public void TC_63861_VerifyProjectApprovalProcessByChoosingApprovalTypeAsDualEitherWhenApprover1_ApprovesAndApprover2_RejectsTheProject(Hashtable<String, String> data) throws Exception {
        ...
        try {
            ...
            ILiglPage page = new LaunchPage()
                    ...
                    .getHeader().goToCasePage().searchcase(data.get("CaseName")).GoToCase(data.get("CaseName"))
                    .getLeftMenu().goToSecurityPage().verifyingApproveStatus().verifyApprovalHistoryStatus(data.get("AssignedUser2"), data.get("Status3"), data.get("AssignedUser1"), data.get("Status1") )
                    .getHeader().logout().login(data.get("Username"), data.get("Password"),data.get("EntitySelection"))
                    .searchcase(data.get("CaseName")).GoToCase(data.get("CaseName")).getLeftMenu().goToSecurityPage().verifyApprovalHistoryStatus(data.get("AssignedUser2"), data.get("Status3"), data.get("AssignedUser1"), data.get("Status1"))
                    .logout();



        } catch (Exception ex) {
            session.log_Error("TC_63861_VerifyProjectApprovalProcessByChoosingApprovalTypeAsDualEitherWhenApprover1_ApprovesAndApprover2_RejectsTheProject Failed");
            throw new Exception("TC_63861_VerifyProjectApprovalProcessByChoosingApprovalTypeAsDualEitherWhenApprover1_ApprovesAndApprover2_RejectsTheProject Failed", ex);
        } finally {
            session.end();
        }
    }
